...
#### Modified for DatapathPipelined (RV32IM core) - gsclib045
...
## Only the gsclib045 standard-cell library and tech/macro LEF are
## needed for this design (no PLL or SRAM macros instantiated).
## Using typical.lib for this first end-to-end test run.
## Switch to slow.lib (or set up MMMC) later for real timing closure.
...
## All RTL files for DatapathPipelined and its sub-modules
read_hdl { DatapathPipelined.v cla.v DividerUnsignedPipelined.v divu_1iter.v }
elaborate $DESIGN
...
## Subdirectories used by report_dp below - created up front so the
## redirected report_dp output doesn't fail with a missing-path error.
